...
const genarateRefreshToken_genarateAccessToken_for_buyer=async(userid)=>{
    ...
    try {
        ...
        if(!buyer) throw new ApiError(404,'Buyer not found while genrating usertokens')
...
const registerBuyer = asyncHandler(async (req, res) => {
    ...
    if (existingBuyer) {
        throw new ApiError(409, 'Buyer with this email or mobile number already exists');
    }
    ...
    const newBuyer = await Buyer.findById(buyer._id).select('-password -refreshToken')
    if (!newBuyer) {
...
const loginBuyer = asyncHandler(async (req, res) => {
    ...
    const {accessToken,refreshToken}=await genarateRefreshToken_genarateAccessToken_for_buyer(buyer._id)
    ...
    return res.status(200)
              ...
              .json(new ApiResponce(200, 'Buyer logged in successfully', {buyer:loggedInBuyer,accessToken, refreshToken}));
})
...
const updateBuyerProfile=asyncHandler(async(req,res)=>{
    ...
    const {firstName, lastName, email, mobileNumber, country, natureOfBuisness}=req.body
    ...
    return res.status(200).json(new ApiResponce(200,'User profile updated successfully',buyer))
})

//get current buyer
const getCurrentBuyer=asyncHandler(async(req,res)=>{

    const buyer=await Buyer.findById(req.buyer?._id).select('-password -refreshToken')
